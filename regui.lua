do
    local function anti_tamper()
        ...
        if type(loadstring) ~= "function" then
            table.insert(issues, "loadstring is not a function");
            return false, issues;
        end;
        if isfunctionhooked and isfunctionhooked(loadstring) then
            table.insert(issues, "loadstring is hooked");
        end;
        if iscclosure and not iscclosure(loadstring) then
            table.insert(issues, "loadstring is not a C closure");
        end;
        if #issues > 0 then
            return false, issues;
        end;
